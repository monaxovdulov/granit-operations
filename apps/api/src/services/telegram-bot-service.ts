...
export class TelegramBotService {
  ...
  private async handleMessage(
    ...
    if (startToken) {
      const result = await this.repository.bindManagerTelegramChat({
        token: startToken,
        providerAccountId,
    ...
    const actor = await this.repository.findManagerTelegramActor({
      providerAccountId,
      externalChatId: chatId,
      externalUserId: from ? String(from.id) : undefined,
      username: from?.username,
    ...
    if (actor) {
      if (isCancelCommand(text)) {
        await this.repository.clearManagerTelegramReplyContext({
      ...
      try {
        await this.repository.persistManagerTelegramReply({
          managerUserId: actor.managerUserId,
    ...
    await this.repository.acceptInboundMessage(
  ...
  private async handleCallbackQuery(
    ...
    if (action.kind === "takeover") {
      await this.repository.takeoverConversationByPublicId({
    ...
    try {
      const context = await this.repository.createManagerTelegramReplyContext({
        managerUserId: actor.managerUserId,
        managerTelegramBindingId: actor.bindingId,
        publicConversationId: action.publicConversationId
      });
