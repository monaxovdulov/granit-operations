...
export function buildWidgetAiDialogueControlReply(
  ...
  if (signals.memorialForOnePerson && hasRecentUncertainty(input)) {
    return {
      text:
        "Понял: речь о памятнике для дедушки, поэтому как ориентир можно начать с варианта для одного человека. Это пока не окончательный выбор — показать несколько простых вариантов без терминов?",
      fallbackMode: "none",
...
function isFrustration(value: string): boolean {
  return /(?:я\s*ж(?:е)?\s+(?:сказал|говорил)|ты\s+(?:че|ч[ёе]|что)|опять\s+(?:то\s+же|повтор)|одно\s+и\s+то\s+же|сколько\s+можно|не\s+(?:слышишь|понимаешь)|уже\s+устал|хватит\s+спрашивать)/u.test(
    value
  );
}
