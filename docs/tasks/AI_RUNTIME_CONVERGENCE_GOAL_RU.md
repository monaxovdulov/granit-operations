...
Статус: `understanding_verified`; все технические срезы приняты и опубликованы,
общий owner teach-back завершён 2026-08-24. Goal запущена 2026-08-04. Обязательный
...
## 12. Teach-back и закрытие

Владелец своими словами подтвердил:

- один runtime уменьшает места расхождения поведения, ценой отсутствия
  скрытого запасного контура;
- ошибочный или устаревший ответ не отправляется, старая попытка отсекается, а
  следующий ход собирается по свежему состоянию;
- после manager takeover AI не отвечает;
- надёжность подтверждается тестами, но тесты не заменяют ручную проверку;
- изменение откатывается целым срезом;
- staging/production deploy разрешается только отдельным ручным решением
  владельца.

Все шесть обязательных измерений playbook — architecture path, компромиссы,
failure paths, evidence, rollback и deploy boundary — проверены. Goal закрыта
как `understanding_verified`. Следующая отдельно активированная Goal:
`AI-LAYER-SIMPLIFICATION`.

## 13. Исторический prompt для новой сессии
